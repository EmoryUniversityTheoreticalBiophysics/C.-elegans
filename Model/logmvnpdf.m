...
%This function is modified from MVNPDF. Everything is the same except the
%last line.

%LOGMVNPDF Log Multivariate normal probability density function (pdf).
%   Y = MVNPDF(X) returns the log probability density of the multivariate normal
...
%   N-by-1 vector.
%
%   Y = MVNPDF(X,MU) returns the log density of the multivariate normal
...
%   match the size of X.
%
%   Y = MVNPDF(X,MU,SIGMA) returns the log density of the multivariate normal
...
% 
% %MVNPDF: return the actual pdf
% y = exp(-0.5*quadform - logSqrtDetSigma - d*log(2*pi)/2);

%Return log of the pdf
